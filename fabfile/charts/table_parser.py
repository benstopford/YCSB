...
column_defs = {'node': {'index': increment(), 'Name': 'Node Id', 'dtype': 'object'},
               ...
               'threadcount': {'index': increment(), 'Name': 'Threadcount', 'dtype': 'int32'},
               ...
               'reconnections': {'index': increment(), 'Name': 'Reconnections', 'dtype': 'float32'},
               'runtime': {'index': increment(), 'Name': 'Runtime', 'dtype': 'float32'}
}
        ...
        if file.endswith(".out"):
            ...
            for line in f:
                if line.startswith('Command line: '):
                    ...
                    p = re.compile("threadcount=[0-9]*")
                    result[column_defs['threadcount']['index']] = p.finditer(line).next().group().split('=')[1]
                ...
                if line.startswith('[OVERALL]') and 'RunTime' in line:
                    result[column_defs['runtime']['index']] = line.split(", ")[-1][:-1]

                if line.startswith('[OVERALL]') and 'Reconnections' in line:
                    result[column_defs['reconnections']['index']] = line.split(", ")[-1][:-1]
